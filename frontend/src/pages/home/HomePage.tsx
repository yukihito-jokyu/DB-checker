...
export function HomePage() {
  ...
  const readyText =
    loadState === "error" ? "Unavailable" : status?.ready ? "Ready" : "Not ready";
    ...
    <main className="grid min-h-screen place-items-center bg-[#f5f7fb] p-8 text-[#17202a]">
      <section className="w-full max-w-[680px] rounded-lg border border-[#d9e0ea] bg-white p-8 shadow-[0_18px_45px_rgba(23,32,42,0.08)]">
        ...
        <p className="mt-4 text-[15px] leading-7 text-[#526171]">
          ローカル DB にダミーデータを投入し、DB
          設計の有効性を検証するための Wails アプリです。
        </p>
        <dl className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2" aria-label="Application status">
          <div className="rounded-lg border border-[#d9e0ea] p-3.5">
            <dt className="text-xs font-bold uppercase text-[#526171]">Status</dt>
            <dd className="mt-1.5 text-base font-bold text-[#17202a]">
              {loadState === "loading" ? "Checking" : readyText}
            </dd>
          </div>
          <div className="rounded-lg border border-[#d9e0ea] p-3.5">
            <dt className="text-xs font-bold uppercase text-[#526171]">Version</dt>
            <dd className="mt-1.5 text-base font-bold text-[#17202a]">
              {versionText}
            </dd>
          </div>
        ...
        <div className="mt-6">
          <Button type="button" onClick={loadStatus} disabled={loadState === "loading"}>
